...
class CreateActualisationWorkerTest {
	...
	void executeWithTask() {
		...
		verify(externalTaskServiceMock).complete("task-1", "create-actualisation-worker", emptyMap());
	}
	...
	@Test
	void executeWithMissingErrandId() {
		final var queryBuilder = mock(ExternalTaskQueryBuilder.class);
		final var topicBuilder = mock(ExternalTaskQueryTopicBuilder.class);
		final var task = mock(LockedExternalTask.class);
		...
		verify(externalTaskServiceMock).handleFailure("task-2", "create-actualisation-worker",
	...
	void executeWithException() {
		...
		verify(externalTaskServiceMock).handleFailure("task-3", "create-actualisation-worker", "test error", 0, 0L);
	}
}
