...
public class CreateErrandWorker extends AbstractTopicWorker {
	...
	protected Map<String, Object> handle(final LockedExternalTask task) {
		final var errand = new Errand()
			...
			.priority(optionalVariable(task, VAR_PRIORITY, String.class).orElse(DEFAULT_PRIORITY))
		...
		optionalVariable(task, VAR_CATEGORY, String.class).ifPresent(errand::category);
		optionalVariable(task, VAR_TYPE, String.class).ifPresent(errand::type);

		final var response = careManagementClient.createErrand(
